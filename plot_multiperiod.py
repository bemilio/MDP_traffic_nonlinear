...
f = open('/Users/ebenenati/surfdrive/TUDelft/Simulations/MDP_traffic_nonlinear/28_Sep_22/saved_test_result_multiperiod.pkl', 'rb')
...
x_store, visited_nodes, road_graph, edge_time_to_index, node_time_to_index, T_horiz_tested, T_simulation, \
...
index_datapoint =0
for T_horiz in T_horiz_tested:
    distance_over_time =np.zeros(T_simulation)
...
N_datapoints = N_tests * len(T_horiz_tested)
list_tests = np.zeros(N_datapoints)
list_T_horiz = np.zeros(N_datapoints)
list_value = np.zeros(N_datapoints)
index_datapoint =0
